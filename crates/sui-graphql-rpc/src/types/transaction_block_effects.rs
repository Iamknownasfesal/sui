...
impl TransactionBlockEffects {
    ...
    async fn transaction_block(&self) -> Result<Option<TransactionBlock>> {
        match self {
            Self::Stored { stored_tx, .. } => Ok(Some(
                TransactionBlock::try_from(stored_tx.clone()).extend()?,
            )),
            Self::Executed {
                tx_data,
                native,
                events,
            } => Ok(Some(TransactionBlock::Executed {
            ...
            Self::DryRun {
                ...
                native,
                events,
            } => Ok(Some(TransactionBlock::DryRun {
                tx_data: tx_data.clone(),
                effects: native.clone(),
                events: events.clone(),
            })),
